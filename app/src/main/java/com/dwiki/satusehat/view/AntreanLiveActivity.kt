        ...
        statusAntreanViewModel.getStatusAntrean(token!!).observe(this){
            when (it.status){
                ...
                Status.SUCCESS ->{
                    ...
                    binding.tvPerkiraanWaktu.text = "Perkiraan waktu giliran anda : " + (it.data?.data?.perkiraanWaktuTunggu
                        ?: "Tidak ada")
                    binding.tvTanggalPendaftraan.text = "Tanggal Pendaftaran : " + it.data?.data?.tanggalPendaftaran
                    binding.tvWaktuPendaftaraan.text = "Jam pendaftaraan tercata: " + it.data?.data?.waktuPendaftaran
                    ...
                    val antreanResponse = it.data?.data
                    binding.btnBatalkan.setOnClickListener {
                        if (antreanResponse != null) {
                            statusAntreanViewModel.batalkanAntrean(token,antreanResponse.antreanId,antreanResponse.jenisPasien)
                            statusAntreanViewModel.responseBatalkan.observe(this){
                                if (it.status == Status.SUCCESS) SuccessDialog("Berhasil membatalkan antrean").show(supportFragmentManager,"success")
                            }
                        }
                    }
                }
